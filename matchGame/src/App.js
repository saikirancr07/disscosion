...
const initial = {
  ...
  showImage: imagesList[0].imageUrl,
  count: 0,
  seconds: 60,
}
// Replace your code here
class App extends Component {
  ...
  componentDidMount() {
    this.TimerId = setInterval(this.decreaseTheSeconds, 1000)
  }
  ...
  getTheImage = () => {
    ...
    return <img src={showImage} alt="match" className="top-image" />
  }
    ...
    <ul className="list-container">
      {tabsList.map(eachItem => (
        <TabListItem
          key={eachItem.tabId}
          tabDetails={eachItem}
          getTheImagesBasedOnTabId={this.getTheImagesBasedOnTabId}
        />
      ))}
    </ul>
  )
  ...
  findTheImageMatched = imageUrl => {
    ...
    if (showImage === imageUrl) {
      ...
      this.setState({showImage: imagesList[0].imageUrl})
      ...
      <ul className="bottom-list-container">
        {filterList.map(eachItem => (
          <ImagesListItem
            ...
            findTheImageMatched={this.findTheImageMatched}
          />
        ))}
      </ul>
    )
            ...
            <li>
              <div className="score-timer">
                <p className="score-para">
                  Score:<span className="score-count">{count}</span>
                </p>
        ...
        {isShow && (
          <div className="container">
            <div className="full-container">
              {this.getTheImage()}
              {this.getTabsList()}
              {this.getTheImagesFromImagesList()}
            </div>
          </div>
        )}
